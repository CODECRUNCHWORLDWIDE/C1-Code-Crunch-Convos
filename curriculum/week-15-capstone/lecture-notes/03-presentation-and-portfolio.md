...
```mermaid
flowchart LR
  S["Situation - set the scene"] --> T["Task - what needed doing"]
  T --> A["Action - what you did"]
  A --> R["Result - outcome and what you learned"]
```
*The STAR structure for telling a capstone story in an interview.*
...
```mermaid
flowchart BT
  A["Self review with a checklist"] --> B["Peer review with a classmate"]
  B --> C["Public posting"]
  C --> D["Mentor or recruiter review"]
```
*Feedback sources, ranked from easiest to highest quality.*
