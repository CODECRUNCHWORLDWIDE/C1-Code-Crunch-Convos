...
first; open the fold under each question to check, and re-read the relevant
lecture section for anything you missed.
...
**B.** `@app.route("/")` is the canonical form. (C, `@app.get("/")`, is a
...
**A.** Debug mode gives you the auto-reloader and the interactive
...
**C.** The `<int:>` converter only matches integer-shaped segments. If
...
**B.** Query strings live in `request.args`. `request.form` holds POST
...
**C.** `{{ post.title }}` renders with Jinja's default auto-escaping,
...
**A.** Jinja's special `loop` object exposes `loop.index` (1-based),
...
**B.** Flask sessions are stored in a *signed* cookie. The secret key is
...
**C.** The Post / Redirect / Get pattern: after a state-changing POST,
...
**B.** `url_for` reverse-resolves the view function name and parameters
...
**B.** Production runs need a real WSGI server (`gunicorn` is the
