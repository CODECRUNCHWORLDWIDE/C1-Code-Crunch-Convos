...
**C** — `"w"` opens for writing and truncates. `"a"` opens for appending without truncating. `"x"` raises if the file already exists.
...
**B** — `with` calls the file object's `__exit__` method, which closes the file even on exceptions.
...
**C** — Python's default newline translation can corrupt CSV rows that contain embedded newlines inside quoted fields. The `csv` module documents this requirement explicitly.
...
**C** — JSON strings must use double quotes. Single-quoted strings are valid Python but invalid JSON.
...
**B** — `raise ... from ...` is explicit exception chaining. The traceback shows "The above exception was the direct cause of the following exception."
...
**B** — The lecture states Python idiomatically prefers EAFP. (Note that `d.get(key, default)` is also fine in this specific case and arguably better for dict lookups; the question is asking about EAFP vs LBYL in general.)
...
**B** — Catching `Exception` swallows `NameError`, `AttributeError`, etc., turning a clear crash into a mysterious wrong result.
...
**C** — `log.exception` is essentially `log.error` plus the current traceback. Only call it inside an `except` block.
...
**B** — `p.stem` strips the extension. `p.name` keeps it (`'draft.md'`); `p.suffix` returns `'.md'`; `p.parent` returns the containing directory.
