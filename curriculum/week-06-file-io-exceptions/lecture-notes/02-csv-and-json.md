...
```mermaid
flowchart TD
    A["Open input and output csv"] --> B["DictReader reads a row"]
    B --> C{"grade at least 70"}
    C -- "Yes" --> D["DictWriter writes the row"]
    C -- "No" --> E["Skip the row"]
    D --> B
    E --> B
```
*The filter-and-write loop: read each row as a dict, keep only passing grades.*
...
```mermaid
flowchart LR
    A["grades.csv"] --> B["DictReader rows"]
    B --> C["Group grades by class"]
    C --> D["statistics.mean per class"]
    D --> E["json.dumps with indent"]
    E --> F["class-averages.json"]
```
*How raw CSV rows become a JSON file of class averages.*
