...
```mermaid
flowchart TD
  A["score is 76"] --> B{"score >= 90"}
  B -- yes --> G1["grade equals A"]
  B -- no --> C{"score >= 80"}
  C -- yes --> G2["grade equals B"]
  C -- no --> D{"score >= 70"}
  D -- yes --> G3["grade equals C"]
  D -- no --> E{"score >= 60"}
  E -- yes --> G4["grade equals D"]
  E -- no --> G5["grade equals F"]
```
*The elif chain checks conditions top to bottom and stops at the first true one.*
...
```mermaid
flowchart TD
  A["Start can_drive check"] --> B{"person is None"}
  B -- yes --> R1["Return False"]
  B -- no --> C{"missing license"}
  C -- yes --> R2["Return False"]
  C -- no --> D{"age under 16"}
  D -- yes --> R3["Return False"]
  D -- no --> E{"person is banned"}
  E -- yes --> R4["Return False"]
  E -- no --> R5["Return True"]
```
*Guard clauses bail out early instead of nesting one more level for each check.*
