...
├── exercises/
│   ├── README.md
│   ├── exercise-01-linear-regression.md
│   ├── exercise-02-iris-classifier.md
│   ├── exercise-03-train-test-split.md
│   ├── exercise-04-pipeline.md
│   └── exercise-05-confusion-matrix.md
├── challenges/
│   ├── README.md
...
1. **Ensemble methods.** Exercise 5 pits a `DummyClassifier` against logistic regression on a deliberately imbalanced set. Swap the logistic regression for `RandomForestClassifier`, then `GradientBoostingClassifier`, and rebuild the confusion matrix each time. Accuracy will barely move — it is already pinned near the majority-class rate. Watch the false-negative cell instead. Does either model actually find more of the rare class, and what does it give up in false positives to do it?
...
3. **Feature importance.** Take the forest you trained in stretch goal 1, print `model.feature_importances_`, and sort it. Which features actually drove the decisions? Compare that ranking to the logistic regression's coefficients on the same data — they do not always agree, and the disagreement is worth a paragraph.
