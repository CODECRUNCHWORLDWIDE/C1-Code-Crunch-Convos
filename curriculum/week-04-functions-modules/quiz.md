...
9. **C**. `__name__` is set to `"__main__"` when the file is run directly, and to the module name when it is imported. The idiom uses this to run script-only code only when running directly. See Lecture Note 4, section 6.
