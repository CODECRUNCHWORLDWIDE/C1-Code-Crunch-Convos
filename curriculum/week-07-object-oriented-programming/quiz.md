...
10 multiple-choice questions. Answer each one before opening the fold
beneath it. Aim for **8 out of 10** before moving on to Week 8.
...
**B** — `__init__` is the *initializer*. Allocation is `__new__`'s job;
...
**C** — `self` is the current instance, passed implicitly. It is a
...
**A** — `species` is defined in the class body and shared by all
...
**B** — mutable class attributes are shared. Use
...
**C** — Always implement `__repr__`; add `__str__` only when the
...
**B** — The MRO is `D → B → C → A → object`. `B.greet` is found first.
...
**B** — Use inheritance only for true "is-a" relationships; otherwise
compose.
...
**C** — `@classmethod` receives `cls`, so it can construct instances of
...
**A** — `@dataclass` generates `__eq__` from the fields by default.
...
**C** — Name-mangling rewrites `__name` to `_ClassName__name` inside
